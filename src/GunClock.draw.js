...
function makeGunClock(){
	...
	var gunmanX = centerX + parseInt(Math.cos(hourToRadian(hour,minute)) * (clockSize*2/3/2) );
	var gunmanY = centerY - parseInt(Math.sin(hourToRadian(hour,minute)) * (clockSize*2/3/2) );

	var inuX = centerX + parseInt(Math.cos(minuteToRadian(minute,second)) * (clockSize*4/5/2) );
	var inuY = centerY - parseInt(Math.sin(minuteToRadian(minute,second)) * (clockSize*4/5/2) );
...
exports.getGunClock = function(size) {
    return GunClock(size);
}
